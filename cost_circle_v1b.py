"""
Round 29 cost — variant v1b: TIGHT-ORBIT.

Hypothesis: baseline wins ~82% but the orbital pattern (kiter_opposite,
aggro_split) is essentially never satisfied. If we crank these weights
hard enough that the MPC actually forms the pattern, we should:
  - eliminate the rare double-aggro deaths
  - finish faster (fewer timeouts) because both kiters consistently draw
    a separate zealot, leaving the shooter to deliver uncontested DPS

Diff vs cost_circle.py (baseline):
  - kiter_opposite multiplier   4.0  → 12.0
  - aggro_split same-target     8.0  → 20.0
  - aggro_split split reward   -3.0  →  -6.0
"""
...
def compute_cost(state) -> Tuple[float, Dict[str, float]]:
    ...
    # ── EDIT: aggro_split weights tightened (8 → 20, -3 → -6) ──
    if n_z >= 2 and n_alive >= 2:
        ...
        if distinct == 1:
            components['aggro_split'] = 20.0       # was 8.0
        else:
            components['aggro_split'] = -6.0       # was -3.0
    else:
        components['aggro_split'] = 0.0

    # ── EDIT: kiter_opposite multiplier tripled (4 → 12) ──
    if len(kiter_idxs) == 2:
        ...
        if n1 > 0.5 and n2 > 0.5:
            ...
            components['kiter_opposite'] = 12.0 * (cos_angle + 1.0)   # was 4.0
        else:
...
def compute_cost_batch(traj, n_marines: int, n_zealots: int,
    ...
    for t in range(H):
        ...
        # ── EDIT: aggro_split tightened (8 → 20, -3 → -6) ──
        ...
        if n_zealots == 2:
            ...
            aggro = np.where(same_aggro, 20.0,
                             np.where(both_alive, -6.0, 0.0))
        else:
            aggro = np.zeros(N)
        aggro = np.where(active, aggro, 0.0)
        comps['aggro_split'] += aggro

        # ── EDIT: kiter_opposite multiplier 4 → 12 ──
        if n_marines == 3:
