"""
Round 29 cost — variant v1c: AGGRESSIVE SHOOTER.

Hypothesis: ~80% baseline DPS uptime is only ~1 marine shooting on
average. If we make the shooter role much more attractive (heavier
in-range bonus, heavier out-of-range penalty, AND penalize the shooter
when its weapon is on cooldown / it's moving), the MPC should pin one
marine in firing position and let the other two carry kite duty alone.

Diff vs cost_circle.py:
  - shooter_in_range out-of-range mult     4.0  → 10.0
  - shooter_in_range hold reward          -3.0  → -10.0
  - shooter movement penalty               NEW: +5.0 if shooter weapon
                                                  not ready (i.e. moved)
"""
...
def compute_cost(state) -> Tuple[float, Dict[str, float]]:
    ...
    # ── EDIT: shooter weights amplified + new movement penalty ──
    if shooter_dist > MARINE_RANGE:
        components['shooter_in_range'] = 10.0 * (shooter_dist - MARINE_RANGE)   # was 4.0
    elif shooter_dist < 2.0:
        components['shooter_in_range'] = 10.0 * (2.0 - shooter_dist)
    else:
        components['shooter_in_range'] = -10.0                                  # was -3.0
    # NEW: penalize the shooter being on weapon cooldown (= it just moved)
    if not state.marine_weapon_ready[shooter_idx]:
        components['shooter_in_range'] += 5.0
...
def compute_cost_batch(traj, n_marines: int, n_zealots: int,
    ...
    for t in range(H):
        ...
        # ── EDIT: shooter weights amplified ──
        sir = np.where(
            shooter_dist > MARINE_RANGE,
            10.0 * (shooter_dist - MARINE_RANGE),                 # was 4.0
            np.where(shooter_dist < 2.0,
                     10.0 * (2.0 - shooter_dist),
                     -10.0)                                        # was -3.0
        )
        # ── EDIT: shooter movement penalty (+5 if not weapon ready) ──
        shooter_moving = ~mr[arange_N, shooter_idx]
        sir = sir + np.where(shooter_moving & m_alive[arange_N, shooter_idx], 5.0, 0.0)
