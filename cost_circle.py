"""
Cost function for Round 29: 3 Marines vs 2 Zealots, all-melee opponents.

Optimal pattern: two outer marines kite in circles around a stationary
center marine. The center marine never moves (max DPS uptime), the outer
marines draw zealot aggro and orbit on opposite sides so the zealots
chase in a circular pattern they can never catch up with.

Novel components in this file vs the existing cost library:

  - aggro_split        : penalize when both zealots have the same closest
                         marine (single marine gets double-teamed and dies
                         in <2 s of melee). Library elsewhere only handles
                         single-zealot or asymmetric (zealot+stalker) setups.

  - shooter_in_range   : the marine furthest from any zealot is implicitly
                         the shooter. Cost rewards that marine being inside
                         MARINE_RANGE of the nearest zealot AND not in
                         melee — i.e. it can fire without moving.

  - kiter_opposite     : the two non-shooter marines penalized when their
                         angular position around the shooter has cos > -1
                         (i.e. anything other than 180° apart). Forces the
                         orbital "opposite-sides" geometry that keeps the
                         two zealots split.

Standard components that mirror cost_3v2 / cost_function:
  - zealot_hp, marine_survival, kiter_distance, marine_spread, dps_uptime
"""
...
MARINE_RANGE = 5.0
ZEALOT_HP_MAX = 150.0       # 100 hp + 50 shields
MARINE_HP_MAX = 45.0
...
def compute_cost(state) -> Tuple[float, Dict[str, float]]:
    ...
    if n_alive == 0:
        ...
        components['marine_survival'] += 15.0 * (n - n_alive)  # extra penalty per dead marine

    # Early exit if combat is over (no marines or no zealots)
    if n_alive == 0 or n_z == 0:
        for k in ('shooter_in_range', 'kiter_distance', 'aggro_split',
                  'kiter_opposite', 'marine_spread', 'dps_uptime'):
            components[k] = 0.0
        return sum(components.values()), components

    # Per-marine: distance to closest live zealot
    closest_z_dist = []
    for i in live:
    ...
    # ── 3. Emergent shooter role: marine with largest closest-zealot distance ──
    ...
    if shooter_dist > MARINE_RANGE:
        ...
        # too close — shooter is in melee danger
        components['shooter_in_range'] = 10.0 * (2.0 - shooter_dist)
    else:
        components['shooter_in_range'] = -3.0  # reward: in good firing range

    # ── 4. Kiter distance (avoid melee, stay in support range) ──
    kite_cost = 0.0
    for d in kiter_dists:
        if d < 1.5:
            kite_cost += 20.0 * (1.5 - d)        # melee = death
        elif d < 3.0:
            kite_cost += 4.0 * (3.0 - d)         # too close, drift away
        elif d > 7.0:
            kite_cost += 1.5 * (d - 7.0)         # drifted too far, useless
    components['kiter_distance'] = kite_cost

    # ── 5. Aggro split: each zealot should target a different marine ──
    if n_z >= 2 and n_alive >= 2:
        ...
        if distinct == 1:
            components['aggro_split'] = 8.0           # both on same marine: bad
        else:
            components['aggro_split'] = -3.0          # split: good
    else:
        components['aggro_split'] = 0.0

    # ── 6. Kiter opposite-sides geometry around the shooter ──
    if len(kiter_idxs) == 2:
        ...
        if n1 > 0.5 and n2 > 0.5:
            ...
            # cos = -1 → 180° apart (best); cos = +1 → same side (worst)
    ...
    # ── 7. Marine spread (don't stack — splash / focused damage) ──
    if n_alive >= 2:
        min_sep = float('inf')
    ...
    # ── 8. DPS uptime (weapon ready + in range) ──
    dps_reward = 0.0
...
# ─────────────────────────────────────────────────────────────
# Batched form (used by mpc_vectorized_circle.py).
# Mirrors compute_cost above; both must be edited together
# during Eureka cost iteration.
# ─────────────────────────────────────────────────────────────
def compute_cost_batch(traj, n_marines: int, n_zealots: int,
    ...
    Args:
      traj: dict from simulate_batch with keys
        m_pos    (N, H, n_m, 2)
        z_pos    (N, H, n_z, 2)
        m_hp     (N, H, n_m)
        z_hp     (N, H, n_z)
    ...
    Returns:
      total: (N,) per-trajectory cost
      components: dict of (N,) per-component cost (summed over horizon)
    """
    m_pos = traj['m_pos']
    z_pos = traj['z_pos']
    m_hp = traj['m_hp']
    ...
    keys = ('zealot_hp', 'marine_survival', 'shooter_in_range',
            ...
            'marine_spread', 'dps_uptime')
    comps = {k: np.zeros(N) for k in keys}
    arange_N = np.arange(N)

    # All-dead rows produce inf distances which then warn on inf*0; the
    # `active` mask zeroes these rows, so the warnings are not load-bearing.
    ...
    for t in range(H):
        ...
        # 6. kiter_opposite (n_marines == 3 only)
        if n_marines == 3:
